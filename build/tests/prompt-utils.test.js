...
const prompt_builder_1 = require("../src/core/prompt/prompt.builder");
(0, node_test_1.test)('previewText truncates with ellipsis', () => {
    const input = 'a'.repeat(10);
    const out = (0, prompt_builder_1.previewText)(input, 5);
    assert_1.strict.equal(out, 'aaaaa[...]');
});
(0, node_test_1.test)('buildUserPromptText respects system support', () => {
    const system = 'SYS';
    const prompt = 'PROMPT';
    assert_1.strict.equal((0, prompt_builder_1.buildUserPromptText)(system, prompt, true), 'PROMPT');
    assert_1.strict.equal((0, prompt_builder_1.buildUserPromptText)(system, prompt, false), 'SYS\n\nPROMPT');
});
(0, node_test_1.test)('buildContinuationParts structure', () => {
    const parts = (0, prompt_builder_1.buildContinuationParts)('prev', 'orig');
