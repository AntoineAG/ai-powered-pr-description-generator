...
class PullRequestUpdater {
  ...
  private inferCommitTypeScored(diffOutput: string, files: string[], currentTitle: string, subject: string): string {
    ...
    const add = (k: string, n: number, reason: string) => { scores[k] += n; const msg = `+${n} => ${k} :: ${reason}`; reasons.push(msg); core.debug(`[Title] score ${msg}`); };
  ...
  private formatConventionalCommitTitle(subject: string, diffOutput: string, files: string[], currentTitle: string): string {
    ...
    if (!type || !allowedTypes.has(type)) {
      ...
      if (type === 'fix' && recommendedType === 'feat') { typeDecisionReason = 'nudged fix->feat due to broader signals'; type = 'feat'; }
      if (type === 'chore' && recommendedType === 'feat') { typeDecisionReason = 'nudged chore->feat because code changes detected'; type = 'feat'; }
    }
